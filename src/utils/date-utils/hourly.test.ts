...
import { isHourlyFormat, findFirstDateTimeForSelectedDay } from './hourly';
...
describe('hourly utils', () => {
  ...
  describe('findFirstHourlyDateForDay', () => {
    ...
    it('should find the first hourly entry for a given day', () => {
      const result = findFirstDateTimeForSelectedDay(mockDates, '20240318', DateFormat.HOUR);
      expect(result).toBe('2024031800');
    });

    it('should return undefined if no entries found for the day', () => {
      const result = findFirstDateTimeForSelectedDay(mockDates, '20240320', DateFormat.HOUR);
      expect(result).toBeUndefined();
    });

    it('should handle empty dates array', () => {
      const result = findFirstDateTimeForSelectedDay([], '20240318', DateFormat.HOUR);
      expect(result).toBeUndefined();
    });
    ...
    it('should handle dates in different formats', () => {
      ...
      const result = findFirstDateTimeForSelectedDay(datesInDifferentFormat, '20240318', DateFormat.HOUR);
